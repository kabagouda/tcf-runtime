...
async function submitCandidate(candidateData) {
    console.log('🚀 Démarrage de la soumission pour:', candidateData.Legal_first_name);

    // Étape 1 et 2: Cloudflare initial
    console.log('⏳ Tentative de passage Cloudflare initial...');
    let { browser, page } = await passCloudflareTurnstile("https://douala.ifc-tests-examens.com/examens-session/10-test-de-connaissance-du-franais-pour-le-canada");
    ...
    try {
        await page.waitForFunction(
            () => {
              ...
              return Array.from(elements).some(el => el.textContent.includes('TCF-CANADA'));
            },
            { timeout: 2000 }
          );
	...
	await page.type('input[name="utilisateur[firstname]"]', candidateData.prenom); // Prénom
	// Mot de passe
	await page.type(
